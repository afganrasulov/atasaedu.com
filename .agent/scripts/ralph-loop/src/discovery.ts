 ...
 * ```
 */
export function pathToWorkspaceId(filePath: string): string {
 ...
 */
export function findAntigravityProcess(workspacePath?: string): ProcessInfo | null {
    try {
        ...
        const targetWorkspaceId = workspacePath
            ...
            : null;
        let fallback: ProcessInfo | null = null;
        ...
        for (const line of output.split("\n")) {
            ...
            if (isLS && isAG) {
                ...
                if (csrfToken) {
                    const info: ProcessInfo = { pid, csrfToken, extensionServerPort, workspaceId };
 ...
 */
export function getListeningPorts(pid: number): number[] {
    try {
 ...
 */
export function probeGrpcPort(port: number, csrfToken: string): Promise<boolean> {
 ...
 * ```
 */
export async function discover(workspacePath: string): Promise<ClientConfig> {

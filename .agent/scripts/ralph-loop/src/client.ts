...
import type { ClientConfig, ModelName, CascadeMode, PollResult } from "./types.js";
...
export class AntigravityClient {
    ...
    async connect(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if (!connected) reject(new Error("Bağlantı zaman aşımı (5s)"));
            }, 5000);
        });
    }
     ...
     */
    private makeHeaders(rpcPath: string, payloadLength?: number): http2.OutgoingHttpHeaders {
        return {
            ":method": "POST",
     ...
     * ```
     */
    async startCascade(enablePlanning = false): Promise<string> {
        if (!this.client) throw new Error("Bağlantı yok");
        ...
        return new Promise((resolve, reject) => {
            const req = this.client!.request(
                this.makeHeaders("StartCascade", payload.length),
            );
    ...
    async sendMessage(
        ...
        return new Promise((resolve, reject) => {
            const req = this.client!.request(
                this.makeHeaders("SendUserCascadeMessage", payload.length),
            );
            req.on("response", (headers) => {
                if (headers[":status"] === 200) {
                    resolve();
     ...
     */
    async pollForCompletion(cascadeId: string, stableThreshold = 7): Promise<void> {
        if (!this.client) throw new Error("Bağlantı yok");
        ...
        while (true) {
            ...
            const result = await new Promise<PollResult>((resolve, reject) => {
                const req = this.client!.request(
                    this.makeHeaders("GetCascadeTrajectorySteps", payload.length),
                );
                let responseData = Buffer.alloc(0);
                req.on("data", (chunk: Buffer) => {
                    responseData = Buffer.concat([responseData, chunk]);
                ...
                req.on("response", (headers) => {
                    req.on("end", () =>
                        resolve({ status: headers[":status"] as number, data: responseData }),
                    );
                });
                req.on("error", reject);
                req.write(payload);
    ...
    async cancelCascade(cascadeId: string): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            const req = this.client!.request(
                this.makeHeaders("CancelCascadeInvocation", payload.length),
            );
            req.on("response", (headers) => {
                headers[":status"] === 200
                    ? resolve()
     ...
     */
    async deleteCascade(cascadeId: string): Promise<void> {
        if (!this.client) return;
        const payload = ldField(1, cascadeId);
        return new Promise((resolve, reject) => {
            const req = this.client!.request(
                this.makeHeaders("DeleteCascadeTrajectory", payload.length),
            );
            req.on("response", (headers) => {
                headers[":status"] === 200
                    ? resolve()
